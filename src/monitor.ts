    ...
    if (newResults.length > oldResults.length) {
      for (let res in newResults) {
        if (isNewProduct(newResults[res], oldResults)) {
          ...
          newProductHook(
            webhookParams.productName,
            webhookParams.productID,
            webhookParams.stockStatus,
            webhookParams.price,
            webhookParams.updatedTimestamp,
            webhookParams.cartable,
            webhookParams.sale,
            webhookParams.imageURL
          );
        }
      }
      ...
      for (let res in newResults) {
        if (
          newResults[res].parts_visible_stock > 0 &&
          oldResults[res].parts_visible_stock === 0
        ) {
          const newProduct = newResults[res];
          const webhookParams = {
            productName: newProduct.name,
          ...
          newResults[res].price != oldResults[res].price ||
          (newResults[res].on_sale != oldResults[res].on_sale &&
            newResults[res].on_sale === true)
        ) {
          const newProduct = newResults[res];
          const webhookParams = {
            productName: newProduct.name,
